...
<c:when test="${not empty invalidCount}">
	<script>
	alert("Invalid count provided. Count not Updated");
	</script>
	</c:when>
...
<c:if test="${not empty bookToggled }" >
	<script>
	alert("Book toggled successfully");
	</script>
</c:if>
			...
			<div class="card-deck g-3">
				<div class="row card-row justify-content-center">
					...
					<c:forEach items="${books}" var="book">
						<div
							class="card col-xl-2 col-lg-2 col-md-3 col-sm-5 col-xs-4 mx-2">
							...
							<div class="card-body text-center">
								...
								<button type="button" class="btn btn-dark btn-sm view"
					...
					<div class="modal-content">
						...
						<div class="modal-footer justify-content-center">
							<c:choose>
								<c:when test='${fn:contains(session, "admin") }'>
									<button type="button"
										class="btn btn-outline-warning waves-effect mx-3 update">Update
										Count</button>
									<form:form action="updateBookCount/${book.getBookId() }" method="post"
										modelAttribute="updateBook">
										<form:input type="number" path="count"
											class="count text-center" />
										<button type="submit"
											class="btn btn-outline-success waves-effect mx-3 add">Update</button>
									</form:form>
									<form:form modelAttribute="enableDisable" action="enableDisable/${book.getBookId()}">
										<c:choose>
											<c:when test='${fn:contains(book.getStatus(), "E") }'>
												<button type="submit"
													class="btn btn-outline-danger waves-effect mx-3 disable">Disable</button>
											</c:when>
											<c:otherwise>
												<button type="submit"
													class="btn btn-outline-success waves-effect mx-3 disable">Enable</button>
											</c:otherwise>
										</c:choose>
									</form:form>
								</c:when>
								<c:otherwise>
									<c:choose>
										...
										<c:otherwise>
											<button type="button"
												class="btn btn-outline-secondary waves-effect mx-3 disabled">Borrow</button>
										</c:otherwise>
									</c:choose>
								</c:otherwise>
							</c:choose>
